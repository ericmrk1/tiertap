...
final class SessionSyncManager: NSObject, ObservableObject {
    ...
    private var cachedTierGoals: [WatchSyncedTierGoal] = []
    ...
    /// Called on Watch when iPhone pushes an updated wallet tier-goals snapshot.
    var onTierGoalsReceived: (([WatchSyncedTierGoal]) -> Void)?

    /// Called on Watch when a wallet tier goal newly reaches 100%.
    var onTierGoalCompleted: ((WatchTierGoalCompletionEvent) -> Void)?
    ...
    /// iPhone: latest wallet tier goals for inclusion in WC state replies / context.
    var tierGoalsSnapshotProvider: (() -> [WatchSyncedTierGoal])?
    ...
    func pushContext(sessions: [Session], liveSession: Session?) {
        ...
        attachTierGoals(to: &ctx)
        pendingApplicationContext = ctx
        flushPendingApplicationContextIfPossible()
    }

    // MARK: - Wallet tier goals (iPhone → Watch)

    /// Pushes the latest wallet tier-goal rings and mirrors them into the App Group.
    func pushTierGoals(_ goals: [WatchSyncedTierGoal]) {
        cachedTierGoals = goals
        #if os(iOS)
        WatchTierGoalSyncCodec.persistGoalsToAppGroup(goals)
        #endif
        // Fold into the next / pending application context so Watch stays current.
        if var pending = pendingApplicationContext {
            attachTierGoals(to: &pending, goals: goals)
            pendingApplicationContext = pending
            flushPendingApplicationContextIfPossible()
        } else if let payload = makePayload(sessions: cachedSessions, liveSession: cachedLiveSession) {
            var ctx: [String: Any] = ["sessions": payload.sessionsData]
            ctx["liveSession"] = payload.liveData ?? Data()
            if let summary = payload.liveSummary {
                ctx[liveSummaryKey] = summary
            }
            ctx["sessionRemindersEnabled"] = UserDefaults.standard.bool(forKey: reminderEnabledKey)
            let minutes = UserDefaults.standard.integer(forKey: reminderFrequencyMinutesKey)
            ctx["sessionReminderFrequencyMinutes"] = max(1, minutes > 0 ? minutes : 30)
            attachTierGoals(to: &ctx, goals: goals)
            pendingApplicationContext = ctx
            flushPendingApplicationContextIfPossible()
        }
    }

    /// Immediately notifies the Watch that a wallet tier goal just hit 100%.
    func notifyTierGoalCompleted(_ event: WatchTierGoalCompletionEvent) {
        guard let data = WatchTierGoalSyncCodec.encodeCompletion(event) else { return }
        let message: [String: Any] = [
            WatchTierGoalSyncKeys.eventTypeKey: WatchTierGoalSyncKeys.eventTypeTierGoalCompleted,
            WatchTierGoalSyncKeys.eventPayloadKey: data
        ]
    ...
    /// Latest goals known to this process (from last push / received context).
    func latestTierGoals() -> [WatchSyncedTierGoal] {
        if !cachedTierGoals.isEmpty { return cachedTierGoals }
        return WatchTierGoalSyncCodec.loadGoalsFromAppGroup()?.goals ?? []
    }
    ...
    private func replyMetadata(from reply: [String: Any]) -> [String: Any] {
        let skip: Set<String> = [
            ...
            WatchTierGoalSyncKeys.contextPayloadKey
        ]
        var out: [String: Any] = [:]
    ...
    private func attachTierGoals(to ctx: inout [String: Any], goals: [WatchSyncedTierGoal]? = nil) {
        let resolved: [WatchSyncedTierGoal]
        if let goals {
            resolved = goals
        } else if let providerGoals = tierGoalsSnapshotProvider?() {
            resolved = providerGoals
            cachedTierGoals = providerGoals
        } else {
            resolved = cachedTierGoals
        }
        if let data = WatchTierGoalSyncCodec.encodeGoals(resolved) {
            ctx[WatchTierGoalSyncKeys.contextPayloadKey] = data
        }
    }

    private func ingestTierGoals(from payload: [String: Any]) {
        guard let data = payload[WatchTierGoalSyncKeys.contextPayloadKey] as? Data,
              let goals = WatchTierGoalSyncCodec.decodeGoals(from: data) else {
            return
        }
        cachedTierGoals = goals
        #if os(watchOS)
        ...
        DispatchQueue.main.async { [weak self] in
            self?.onTierGoalsReceived?(goals)
        }
        #endif
    }

    /// Handles iPhone → Watch tier-goal celebration payloads (message or userInfo).
    @discardableResult
    private func handleIncomingWatchEvent(from payload: [String: Any]) -> Bool {
        guard let eventType = payload[WatchTierGoalSyncKeys.eventTypeKey] as? String else {
            return false
        }
        if eventType == WatchTierGoalSyncKeys.eventTypeTierGoalCompleted,
           let data = payload[WatchTierGoalSyncKeys.eventPayloadKey] as? Data,
           let event = WatchTierGoalSyncCodec.decodeCompletion(from: data) {
            cachedTierGoals = cachedTierGoals.map { existing in
                existing.id == event.goal.id ? event.goal : existing
            }
            if !cachedTierGoals.contains(where: { $0.id == event.goal.id }) {
                cachedTierGoals.append(event.goal)
            }
            #if os(watchOS)
            WatchTierGoalSyncCodec.persistGoalsToAppGroup(cachedTierGoals)
            DispatchQueue.main.async { [weak self] in
                self?.onTierGoalCompleted?(event)
            }
            #endif
            return true
        }
...
extension SessionSyncManager: WCSessionDelegate {
    ...
    /// iPhone receives message from Watch (action to perform). Watch may also receive celebration events from iPhone.
    func session(_ session: WCSession, didReceiveMessage message: [String: Any], replyHandler: @escaping ([String: Any]) -> Void) {
        #if os(watchOS)
        if handleIncomingWatchEvent(from: message) {
    ...
    /// Queued payloads: Watch→iPhone actions, or iPhone→Watch tier-goal celebrations.
    func session(_ session: WCSession, didReceiveUserInfo userInfo: [String: Any]) {
        #if os(watchOS)
        if handleIncomingWatchEvent(from: userInfo) {
            return
        }
        #endif
