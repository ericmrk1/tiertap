...
enum SupabaseTables {
    ...
    /// Community feed reaction ledger (who reacted). Mirrors `_Test` in simulator / TestFlight.
    static var communityPostReactions: String {
        return shouldUseTestSupabaseTables() ? "CommunityPostReactions_Test" : "CommunityPostReactions"
    }

    /// Denormalized running totals per post (like / dislike / heart). Mirrors `_Test` in simulator / TestFlight.
    static var communityPostReactionCounts: String {
        return shouldUseTestSupabaseTables() ? "CommunityPostReactionCounts_Test" : "CommunityPostReactionCounts"
    }
}
