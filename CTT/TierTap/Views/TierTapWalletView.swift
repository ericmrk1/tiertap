...
struct TierTapWalletView: View {
    ...
    /// Root tab presentation — hides the sheet-style Done button.
        ...
        .overlay(alignment: .bottom) {
            if !cards.isEmpty, useCollapsedDeckLayout {
                addCardFloatingButton
                    .padding(.bottom, walletBottomChromeHeight + 16 + rootTabBottomClearance)
            }
        }
        .onAppear {
    ...
    private var emptyState: some View {
        VStack(spacing: 20) {
            ...
            Spacer()
            addCardFloatingButton
                .padding(.bottom, 28 + rootTabBottomClearance)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    }
    ...
    /// Clears the floating pill tab bar when Wallet is a root tab.
    private var rootTabBottomClearance: CGFloat { isRootTab ? 72 : 0 }
        ...
        ZStack(alignment: .bottom) {
            ...
            if useExpandedSingleCardLayout {
                expandedWalletBottomChrome
                    ...
                    // Keep edit / history / stack controls above the floating pill tab bar.
                    .padding(.bottom, rootTabBottomClearance)
            } else {
                walletBottomChrome
                    .frame(height: walletBottomChromeHeight)
                    .frame(maxWidth: .infinity)
                    // Keep card controls (edit, history, info, chevrons) above the floating pill tab bar.
                    .padding(.bottom, rootTabBottomClearance)
            }
        }
    }
    ...
    private func browseWalletBottomChrome(front: RewardWalletCard?) -> some View {
        VStack(spacing: 10) {
            ...
            browseWalletControlsPill(front: front)
    ...
    /// Edit / trend / info / deck-scroll controls in a floating capsule matching the main tab pill.
    private func browseWalletControlsPill(front: RewardWalletCard?) -> some View {
        let historyActive = front.map { historyOverlayCardID == $0.id } ?? false
        return HStack(spacing: 2) {
            WalletPillIconButton(
                systemImage: "square.and.pencil",
                accessibilityLabel: "Edit card",
                isEnabled: front != nil
            ) {
                if let c = front { cardToEdit = c }
            }
            WalletPillIconButton(
                systemImage: historyActive ? "clock.badge.checkmark.fill" : "chart.line.uptrend.xyaxis",
                accessibilityLabel: historyActive ? "Hide tier history overlay" : "Show tier history overlay",
                isSelected: historyActive,
                isEnabled: front != nil
            ) {
                if let c = front { toggleTierHistoryOverlay(for: c) }
            }
            WalletPillIconButton(
                systemImage: "info.circle.fill",
                accessibilityLabel: "Card details",
            ...
            WalletPillIconButton(
                ...
                isEnabled: safeSelectedIndex < cards.count - 1
            ) {
        ...
        ZStack(alignment: .bottom) {
            ...
            if showsFooterOverlay {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if TierTapProductEnhancements.isEnabled, card.hasTierGoal, let remaining = card.pointsRemainingToTierGoal {
                        let label = card.tierGoalLabel?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
                        Text(remaining == 0
                             ? (label.isEmpty ? "Goal reached" : "\(label) reached")
                             : (label.isEmpty ? "\(remaining) pts to goal" : "\(remaining) to \(label)"))
                            .font(.caption.weight(.semibold))
...
/// Segment control inside the wallet deck action pill (matches main tab bar item styling).
private struct WalletPillIconButton: View {
    let systemImage: String
    let accessibilityLabel: String
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: systemImage)
                ...
                .foregroundStyle(isSelected ? Color.white : Color.white.opacity(isEnabled ? 0.72 : 0.35))
                .frame(maxWidth: .infinity)
                .frame(height: 40)
                .background {
                    if isSelected {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    if TierTapProductEnhancements.isEnabled {
                        if card.hasTierGoal {
                            LoyaltyTierGoalRingView(card: card, compact: true)
                        }
                        detailRow(
                            title: "Tier goal points",
                            value: card.tierGoalPoints.map(String.init) ?? "—"
                        )
                        detailRow(
                            title: "Goal label",
                            value: (card.tierGoalLabel?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
                                ? (card.tierGoalLabel ?? "—")
                                : "—"
                        )
                    }
...
private struct NewWalletCardSheet: View {
    ...
    @State private var tierGoalPointsText = ""
    @State private var tierGoalLabel = ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                if TierTapProductEnhancements.isEnabled {
                    Section {
                        LoyaltyProgramLadderPicker(
                            rewardProgram: $rewardProgram,
                            tierGoalPointsText: $tierGoalPointsText,
                            tierGoalLabel: $tierGoalLabel
                        )
                        DisclosureGroup("Customize goal manually") {
                            NumericEntryWithDialPad(
                                placeholder: "Tier goal points",
                                text: $tierGoalPointsText,
                                dialPadNavigationTitle: "Tier goal points"
                            )
                            TextField("Goal label (e.g. NOIR)", text: $tierGoalLabel)
                        }
                    } header: {
                        Text("Tier goal")
                    } footer: {
                        Text("Ladder presets fill points + label in one tap. Shown as a ring on Home and Wallet.")
                    }
                }
            }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let goalPts = Int(tierGoalPointsText.filter(\.isNumber))
                        let goalLabel = tierGoalLabel.trimmingCharacters(in: .whitespacesAndNewlines)
                        let ok = walletStore.addCard(
                            ...
                            notes: notes,
                            tierGoalPoints: TierTapProductEnhancements.isEnabled ? goalPts : nil,
                            tierGoalLabel: TierTapProductEnhancements.isEnabled && !goalLabel.isEmpty ? goalLabel : nil
                        )
                        if ok {
...
private struct EditWalletCardSheet: View {
    ...
    @State private var tierGoalPointsText: String = ""
    @State private var tierGoalLabel: String = ""
        ...
        NavigationStack {
            Form {
                ...
                if TierTapProductEnhancements.isEnabled {
                    Section {
                        ...
                        Text("Ladder presets fill points + label in one tap. Progress ring appears on Home.")
                    }
                }
                Section {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        updated.notes = notes
                        if TierTapProductEnhancements.isEnabled {
                            updated.tierGoalPoints = Int(tierGoalPointsText.filter(\.isNumber))
                            let label = tierGoalLabel.trimmingCharacters(in: .whitespacesAndNewlines)
                            updated.tierGoalLabel = label.isEmpty ? nil : label
                        }
            ...
            .onAppear {
                ...
                if let goal = card.tierGoalPoints {
                    tierGoalPointsText = "\(goal)"
                }
                tierGoalLabel = card.tierGoalLabel ?? ""
                if let exp = card.expirationDate {
                    hasExpiration = true
                    expirationDate = exp
                } else {
