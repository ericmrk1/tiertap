...
struct SettingsView: View {
    ...
    /// Searchable settings sections (order matches the Settings screen).
    private enum SettingsSearchSection: String, CaseIterable, Identifiable {
        case about
        case account
        case tierTapAI
        case tokens
        ...
        /// Section header key (localized via `L10n.tr`).
        var titleKey: String {
            switch self {
            case .about: return "About"
            case .account: return "Account"
            case .tierTapAI: return "TierTap AI"
            case .tokens: return "TierTap Plus"
            case .bankroll: return "Bankroll & Localization"
            case .riskOfRuin: return "Target average"
            case .favorites: return "Favorites"
            case .sessions: return "Sessions"
            case .theme: return "Theme & colors"
            case .watchExperience: return "Watch Experience"
            case .dataExport: return "Data & Export"
            }
        }

        /// Primary setting labels shown in type-ahead suggestions.
        var settingLabels: [String] {
            switch self {
            case .about:
                return ["User Guide", "Leave Feedback", "Privacy Policy", "Terms of Use (EULA)", "Sponsor"]
            case .account:
                return ["Subscriptions", "TierTap Pro", "TierTap Account", "App lock"]
            case .tierTapAI:
                return ["Tone of voice", "Typing speed", "Ask TierTap"]
            case .tokens:
                return ["Buy tokens", "Token usage", "Sessions chart", "API tokens"]
            case .bankroll:
                return ["Bankroll", "Currency", "App language"]
            case .riskOfRuin:
                return ["Target win per session", "Target average"]
            case .favorites:
                return [
                    "Favorite casino games", "Favorite slot games", "Default game type",
                    "Common casino locations", "Common denominations", "Casino chimes & haptics",
                    "Sound profile",
                ]
            case .sessions:
                return ["Prompt for session mood", "Play Reminders", "Monthly Wrapped notifications"]
            case .theme:
                return ["Theme presets", "Custom colors", "Home widget metrics", "Show tab bar labels"]
            case .watchExperience:
                return [
                    "Watch haptics", "Watch animations", "Haptic profile",
                    "Session pulse reminders", "Wrist-raise live summary", "Watch buy-in defaults",
                ]
            case .dataExport:
                return ["CSV game type", "Export sessions as CSV"]
            }
        }

        /// Extra aliases used for matching (not always shown as suggestions).
        var searchAliases: [String] {
            switch self {
            case .about:
                return ["Feedback", "EULA", "Gamblers Anonymous", "help", "about"]
            case .account:
                return ["Upgrade", "Manage TierTap Pro", "sign in", "login", "security", "password"]
            case .tierTapAI:
                return ["TierTap AI", "AI", "sarcastic", "summarize", "summary"]
            case .tokens:
                return ["TierTap Plus", "Tokens", "credits", "token pack", "purchase", "usage", "chart"]
            case .bankroll:
                return ["Localization", "language", "money", "Risk of Ruin", "symbol"]
            case .riskOfRuin:
                return ["goal", "average win", "Risk of Ruin"]
            case .favorites:
                return ["Favorites", "denominations", "sounds", "casinos", "slots", "poker", "eighteen", "18x", "haptics"]
            case .sessions:
                return ["Sessions", "mood", "reminders", "notifications", "bell", "Wrapped", "Monthly Wrapped"]
            case .theme:
                return ["Theme", "colors", "Presets", "Primary", "Secondary", "appearance", "gradient", "widget", "tab bar", "labels"]
            case .watchExperience:
                return ["Watch Experience", "Apple Watch", "buy-in", "comp", "watch", "pulse"]
            case .dataExport:
                return ["Data & Export", "CSV", "export", "backup", "share"]
            }
        }

        func matches(query: String, language: AppLanguage) -> Bool {
            !matchingSuggestions(query: query, language: language).isEmpty
                || titleMatches(query: query, language: language)
        }

        func titleMatches(query: String, language: AppLanguage) -> Bool {
            let q = query.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !q.isEmpty else { return true }
            return L10n.tr(titleKey, language: language).localizedStandardContains(q)
                || titleKey.localizedStandardContains(q)
        }

        func matchingSuggestions(query: String, language: AppLanguage) -> [SettingsSearchSuggestion] {
            let q = query.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !q.isEmpty else { return [] }

            var results: [SettingsSearchSuggestion] = []
            if titleMatches(query: q, language: language) {
                results.append(
            ...
            for label in settingLabels {
                let localized = L10n.tr(label, language: language)
                if localized.localizedStandardContains(q) || label.localizedStandardContains(q) {
                    results.append(
                        SettingsSearchSuggestion(
                            id: "\(rawValue)|\(label)",
                            section: self,
            ...
            // Alias-only hits still surface the section once.
            if results.isEmpty {
                let aliasHit = searchAliases.contains { alias in
                    alias.localizedStandardContains(q)
                        || L10n.tr(alias, language: language).localizedStandardContains(q)
                }
                if aliasHit {
                    results.append(
                        SettingsSearchSuggestion(
    ...
    private struct SettingsSearchSuggestion: Identifiable {
        let id: String
        let section: SettingsSearchSection
        let labelKey: String
    }
    ...
    @FocusState private var isSettingsSearchFieldFocused: Bool

    private var trimmedSettingsSearchQuery: String {
        settingsSearchQuery.trimmingCharacters(in: .whitespacesAndNewlines)
    }

    private var displayedSettingsSections: [SettingsSearchSection] {
        let language = settingsStore.appLanguage
        guard !trimmedSettingsSearchQuery.isEmpty else {
            return Array(SettingsSearchSection.allCases)
        }
        return SettingsSearchSection.allCases.filter {
            $0.matches(query: settingsSearchQuery, language: language)
        }
    }

    private var settingsTypeAheadSuggestions: [SettingsSearchSuggestion] {
        let language = settingsStore.appLanguage
        guard !trimmedSettingsSearchQuery.isEmpty else { return [] }
        var seen = Set<String>()
        var out: [SettingsSearchSuggestion] = []
        for section in SettingsSearchSection.allCases {
            for suggestion in section.matchingSuggestions(query: settingsSearchQuery, language: language) {
    ...
    var body: some View {
        NavigationStack {
            ScrollViewReader { scrollProxy in
                ScrollView {
                    VStack(spacing: 24) {
                        if displayedSettingsSections.isEmpty, !trimmedSettingsSearchQuery.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                L10nText("No matching settings.")
                                    .font(.headline)
                            ...
                            ForEach(displayedSettingsSections) { section in
                                settingsSectionView(section)
                                    .id(section.id)
                            }
                        }

                        // Extra scroll room past the last section / floating tab pill.
                        Color.clear
                            .frame(height: 96)
                ...
                .safeAreaInset(edge: .top, spacing: 0) {
                    settingsSearchChrome(scrollProxy: scrollProxy)
                }
                .onChange(of: settingsSearchQuery) { newValue in
                    applySettingsSearch(query: newValue, scrollProxy: scrollProxy)
                }
            }
            .background {
    ...
    @ViewBuilder
    private func settingsSectionView(_ section: SettingsSearchSection) -> some View {
        switch section {
        case .about: aboutSection
        case .account: accountSection
        case .tierTapAI: tierTapAISection
        case .tokens: tokensSection
        case .bankroll: bankrollSection
        case .riskOfRuin: riskOfRuinSection
        case .favorites: favoritesSection
        case .sessions: sessionsSection
        case .theme: themeSection
        case .watchExperience: watchExperienceSection
        case .dataExport: dataExportSection
        }
    }

    private func expansionBinding(for section: SettingsSearchSection) -> Binding<Bool> {
        switch section {
        case .about: return $isAboutExpanded
        case .account: return $isAccountExpanded
        case .tierTapAI: return $isTierTapAIExpanded
        case .tokens: return $isTokensExpanded
        case .bankroll: return $isBankrollExpanded
        case .riskOfRuin: return $isRiskOfRuinExpanded
        case .favorites: return $isFavoritesExpanded
        case .sessions: return $isSessionsExpanded
        case .theme: return $isThemeExpanded
        case .watchExperience: return $isWatchExperienceExpanded
        case .dataExport: return $isDataExportExpanded
        }
    }

    private func setAllSettingsSectionsExpanded(_ expanded: Bool) {
        for section in SettingsSearchSection.allCases {
            expansionBinding(for: section).wrappedValue = expanded
        }
    }

    private func applySettingsSearch(query: String, scrollProxy: ScrollViewProxy) {
        let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines)
        let language = settingsStore.appLanguage
        let matched = SettingsSearchSection.allCases.filter {
            $0.matches(query: query, language: language)
        }
        if trimmed.isEmpty {
            setAllSettingsSectionsExpanded(false)
            return
        }
        withAnimation(.easeInOut) {
            for section in SettingsSearchSection.allCases {
                expansionBinding(for: section).wrappedValue = matched.contains(section)
            }
        }
        if let firstId = matched.first?.id {
    ...
    private func jumpToSettingsSuggestion(
        _ suggestion: SettingsSearchSuggestion,
        scrollProxy: ScrollViewProxy
    ) {
        withAnimation(.easeInOut) {
            for section in SettingsSearchSection.allCases {
                expansionBinding(for: section).wrappedValue = (section == suggestion.section)
            }
        }
        isSettingsSearchFieldFocused = false
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.06) {
            withAnimation(.easeInOut(duration: 0.35)) {
                scrollProxy.scrollTo(suggestion.section.id, anchor: .top)
            }
        }
    }

    private func settingsSearchChrome(scrollProxy: ScrollViewProxy) -> some View {
        VStack(spacing: 8) {
            settingsSearchBubble

            if !settingsTypeAheadSuggestions.isEmpty {
                VStack(spacing: 0) {
                    ForEach(settingsTypeAheadSuggestions) { suggestion in
                        Button {
                            jumpToSettingsSuggestion(suggestion, scrollProxy: scrollProxy)
                        } label: {
                            HStack(spacing: 10) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(L10n.tr(suggestion.labelKey, language: settingsStore.appLanguage))
                                        .font(.subheadline.weight(.semibold))
                                        .foregroundColor(.white)
                                        .multilineTextAlignment(.leading)
                                    if suggestion.labelKey != suggestion.section.titleKey {
                                        Text(L10n.tr(suggestion.section.titleKey, language: settingsStore.appLanguage))
                                            .font(.caption2)
                                            .foregroundColor(.white.opacity(0.55))
                                    }
                                }
                        ...
                        if suggestion.id != settingsTypeAheadSuggestions.last?.id {
        ...
        .padding(.bottom, 6)
        .background(settingsStore.primaryGradient.opacity(0.98))
    }

    private var settingsSearchBubble: some View {
        HStack(spacing: 10) {
            ...
            TextField(
                ...
                prompt: Text(L10n.tr("Search settings…", language: settingsStore.appLanguage))
                    .foregroundColor(.white.opacity(0.45))
            )
            .focused($isSettingsSearchFieldFocused)
            ...
            if !settingsSearchQuery.isEmpty {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(L10n.tr("Clear settings search", language: settingsStore.appLanguage))
            }
        }
        .padding(.horizontal, 14)
        .padding(.vertical, 12)
    ...
    private var sessionsSection: some View {
        SettingsSection(
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Toggle(isOn: $settingsStore.monthlyWrappedNotificationsEnabled) {
                    L10nText("Monthly Wrapped notifications")
                }
                .tint(.green)
                .onChange(of: settingsStore.monthlyWrappedNotificationsEnabled) { enabled in
                    if !enabled {
                        TierTapProductEnhancements.cancelPendingMonthlyWrappedNotifications()
                    }
                }
                L10nText("When on, TierTap can notify you when last month’s Wrapped recap is ready. Turn off to stop these alerts; you can still open Monthly Wrapped from History.")
                    .font(.caption)
                    .foregroundColor(.gray)
    ...
    private var themeSection: some View {
        SettingsSection(
            ...
            VStack(alignment: .leading, spacing: 14) {
                Toggle(isOn: $settingsStore.showTabBarLabels) {
                    L10nText("Show tab bar labels")
                }
                .tint(.green)
                Text("Display one-word labels below each tab icon in the main navigation bar.")
                    .font(.caption)
                    .foregroundColor(.gray)
