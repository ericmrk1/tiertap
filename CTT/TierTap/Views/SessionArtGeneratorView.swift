...
let keySessionArtSharePreset = "ctt_session_art_share_preset_v1"

struct SessionArtSharePreset: Codable {
...
enum SessionUnderlaySource: Hashable, Identifiable {
    case uploaded
    case chipEstimator
    case compPhoto(UUID)
...
let sessionArtReferenceWidth: CGFloat = 1080
let sessionArtExportCanvas = CGSize(width: 2160, height: 3840)

enum MetricLineKey: String, Hashable, CaseIterable, Codable {
    case tierBump
    case buyIn
    case cashOut
    case winLoss
    ...
    var displayName: String {
        switch self {
        case .tierBump: return "Tier bump"
        case .buyIn: return "Buy-in"
        case .cashOut: return "Cash out"
        case .winLoss: return "Win / Loss"
        case .winRate: return "Win rate"
        case .tiersPerHour: return "Tiers / hour"
        case .comps: return "Comps"
        }
    }
}

enum SessionArtHeaderFocus: String, Equatable, Codable {
    case casino
    case game
}

enum SessionArtTextFont: String, CaseIterable, Identifiable {
    case system
    case rounded
    case serif
    case mono
...
enum SessionArtColorToken: String, CaseIterable, Identifiable {
    case white
    case black
    case mint
    case yellow
...
enum SessionArtPickerGroup: String, CaseIterable, Identifiable {
...
enum SessionArtBorderStyle: String, Equatable, Codable {
    case none
    case vintagePaper
    case matteFrame
    case artDeco
...
enum SessionArtStickerOverlayStyle: String, Equatable, Codable {
    case none
    case circle
    case triangle
    case square
...
struct SessionArtLayout: Equatable {
    ...
    /// Per-metric scale multipliers (1 = default). Used by the live canvas editor.
    var lineScales: [MetricLineKey: CGFloat] = [:]
    /// Per-metric rotation in radians. Used by the live canvas editor.
    var lineRotations: [MetricLineKey: CGFloat] = [:]
    /// Hidden metric layers (WP-E5). Missing keys are visible.
    var hiddenLines: [MetricLineKey: Bool] = [:]
    /// Locked metric layers — gestures ignored (WP-E5).
    var lockedLines: [MetricLineKey: Bool] = [:]
    /// Hide the headline / session title layer.
    var headerHidden: Bool = false
    /// Lock the headline layer against drag.
    var headerLocked: Bool = false
    /// Hide the caption / footnote layer.
    var footerHidden: Bool = false
    /// Lock the caption layer against drag.
    var footerLocked: Bool = false

    func isLineHidden(_ key: MetricLineKey) -> Bool { hiddenLines[key] == true }
    func isLineLocked(_ key: MetricLineKey) -> Bool { lockedLines[key] == true }

    mutating func setLineHidden(_ key: MetricLineKey, _ hidden: Bool) {
        if hidden { hiddenLines[key] = true } else { hiddenLines.removeValue(forKey: key) }
    }

    mutating func setLineLocked(_ key: MetricLineKey, _ locked: Bool) {
        if locked { lockedLines[key] = true } else { lockedLines.removeValue(forKey: key) }
    }
...
enum SessionArtTemplate: String, CaseIterable, Identifiable {
    case balanced
    case spotlight
    case railRight
    case lowerThird
...
enum SessionArtRenderer {
    ...
    struct RenderParams {
        ...
        /// Optional free-text headline (WP-E4). When non-empty, replaces the auto session title.
    ...
    private static func drawHeader(params: RenderParams, layout: SessionArtLayout, in size: CGSize, cg: CGContext) {
        ...
        let custom = params.customHeadline.trimmingCharacters(in: .whitespacesAndNewlines)
        let titleString = custom.isEmpty ? header.title : custom
        ...
        let title = titleString as NSString
        let titleWidth = title.size(withAttributes: attrs).width
        let titleX = max(safe.minX, min(safe.maxX - titleWidth, origin.x))
        ...
        // Keep session sublines when using a custom headline so date/game context remains.
        for line in header.sublines {
            let ns = line as NSString
            let lineWidth = ns.size(withAttributes: subAttrs).width
            let x = max(safe.minX, min(safe.maxX - lineWidth, origin.x))
    ...
    private static func drawMetricsBlock(
        ...
        func paintMetricLine(title: String, value: String, key: MetricLineKey) {
            ...
            let lineScale = min(2.5, max(0.45, layout.lineScales[key] ?? 1))
            let lineRotation = layout.lineRotations[key] ?? 0
            let pivot = CGPoint(x: (minX + maxX) * 0.5, y: (safeY + valueBaseline + valueFont.lineHeight) * 0.5)
            cg.saveGState()
            if abs(lineRotation) > 0.001 || abs(lineScale - 1) > 0.001 {
                cg.translateBy(x: pivot.x, y: pivot.y)
                cg.rotate(by: lineRotation)
                cg.scaleBy(x: lineScale, y: lineScale)
                cg.translateBy(x: -pivot.x, y: -pivot.y)
            }
...
private struct SessionArtPreviewSheet: View {
    ...
    /// Optional free-text headline override.
        ...
        case .image:
            VStack(spacing: 8) {
                ...
                HStack(spacing: 10) {
                    ...
                    Button {
                        guard let renderedPreviewImage else { return }
                        SessionArtPhotoLibrarySaver.saveImage(renderedPreviewImage) { ok in
                            saveMessage = ok ? "Saved to Photos." : "Couldn't save to Photos."
                        }
                    } label: {
                    ...
                    .buttonStyle(.plain)
                    .disabled(renderedPreviewImage == nil)
                }
            }
        }
    }
...
struct SessionArtGeneratorView: View {
    let sessionId: UUID
    let onBack: () -> Void
    /// Optional raw OutputKind value ("Image", "Graph", "Text", "TierTap AI").
    var initialOutputKindRaw: String? = nil
    ...
    @State private var privacyPreset: SessionArtPrivacyPreset = SessionArtSharePresetStore.lastPrivacyPreset
    @State private var aspectRatio: SessionArtAspectRatio = SessionArtSharePresetStore.lastAspectRatio
    @State private var showLiveCanvas = false
    @State private var liveCanvasShareItem: SessionArtShareMediaItem?
    @State private var liveCanvasStatus: String?
    @State private var hasAutoPickedUnderlay = false
    ...
    @State private var previewCustomHeadline = ""
    ...
    private var designCanvas: CGSize { aspectRatio.canvasSize }

    init(sessionId: UUID, initialOutputKind: String? = nil, onBack: @escaping () -> Void) {
        self.sessionId = sessionId
        self.onBack = onBack
        self.initialOutputKindRaw = initialOutputKind
    }

    /// Compatibility for call sites using trailing-closure only.
    init(sessionId: UUID, onBack: @escaping () -> Void) {
        self.sessionId = sessionId
        self.onBack = onBack
        self.initialOutputKindRaw = nil
    }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPreviewSheet) {
            if let session = resolvedSession {
                SessionArtPreviewSheet(
                    ...
                    customHeadline: previewCustomHeadline,
        ...
        .fullScreenCover(isPresented: $showLiveCanvas) {
            if let session = resolvedSession {
                SessionArtLiveCanvasEditor(
                    session: session,
                    layout: $previewLayout,
                    ...
                    footerCaption: $previewFooterCaption,
                    customHeadline: $previewCustomHeadline,
                    underlay: loadUnderlayImageForStudio(session: session),
                    currencySymbol: settingsStore.currencySymbol,
                    onShare: { image in
                        saveSharePreset()
                        SessionArtSharePresetStore.lastPrivacyPreset = privacyPreset
                        SessionArtSharePresetStore.lastAspectRatio = aspectRatio
                        liveCanvasShareItem = SessionArtShareMediaItem(activityItems: [image])
                    },
                    onShareItems: { items in
                        saveSharePreset()
                        liveCanvasShareItem = SessionArtShareMediaItem(activityItems: items)
                    },
                    onShareTransparent: { image in
                        liveCanvasShareItem = SessionArtShareMediaItem(activityItems: [image])
                    },
                    onShareAnimated: { url in
                        liveCanvasShareItem = SessionArtShareMediaItem(activityItems: [url])
                    },
                    onShareInstagram: { image in
                        InstagramStoriesSharer.shareImage(image) { ok in
                            if !ok {
                                liveCanvasShareItem = SessionArtShareMediaItem(activityItems: [image])
                                liveCanvasStatus = "Instagram unavailable — opened system share."
                            }
                        }
                    },
                    onSavePhotos: { image in
                        SessionArtPhotoLibrarySaver.saveImage(image) { ok in
                            liveCanvasStatus = ok ? "Saved to Photos." : "Couldn't save to Photos."
                        }
                    },
                    onCancel: { showLiveCanvas = false }
                )
            }
        }
        .sheet(item: $liveCanvasShareItem) { item in
            ShareSheet(items: item.activityItems)
        }
        .alert("Studio", isPresented: Binding(
            get: { liveCanvasStatus != nil },
            set: { if !$0 { liveCanvasStatus = nil } }
        )) {
            Button("OK", role: .cancel) {}
        } message: {
            Text(liveCanvasStatus ?? "")
        }
        .onAppear {
            applySavedSharePresetIfNeeded()
            applyInitialOutputKindIfNeeded()
            applyPrivacyPreset(privacyPreset)
            autoPickUnderlayIfNeeded()
    ...
    private var privacyPresetPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Privacy")
                .font(.caption.bold())
                .foregroundColor(.gray)
            Picker("", selection: $privacyPreset) {
                ForEach(SessionArtPrivacyPreset.allCases) { p in
                    Text(p.title).tag(p)
                }
            }
            .pickerStyle(.segmented)
            .onChange(of: privacyPreset) { preset in
                SessionArtSharePresetStore.lastPrivacyPreset = preset
                applyPrivacyPreset(preset)
            }
            Text(privacyPreset.subtitle)
                .font(.caption2)
    ...
    private var aspectRatioPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            .pickerStyle(.segmented)
            .onChange(of: aspectRatio) { ratio in
                SessionArtSharePresetStore.lastAspectRatio = ratio
                if let session = resolvedSession {
                    previewLayout = composedLayout(for: session, canvasSize: ratio.canvasSize)
                }
            }
        }
    }

    private var liveStudioButton: some View {
        Button {
            if let session = resolvedSession {
                previewLayout = composedLayout(for: session, canvasSize: aspectRatio.canvasSize)
            }
            showLiveCanvas = true
        } label: {
            Label("Open live studio", systemImage: "hand.draw")
                .font(.subheadline.weight(.semibold))
                .frame(maxWidth: .infinity)
                .padding(.vertical, 12)
    ...
    private func applyPrivacyPreset(_ preset: SessionArtPrivacyPreset) {
        ...
        artStyle = preset.includeMetrics ? .photoWithMetrics : .photoOnly
    }

    private func applyInitialOutputKindIfNeeded() {
        guard let raw = initialOutputKindRaw,
              let kind = OutputKind(rawValue: raw) else { return }
        outputKind = kind
    }

    private func autoPickUnderlayIfNeeded() {
        guard !hasAutoPickedUnderlay else { return }
        hasAutoPickedUnderlay = true
        guard selectedUnderlay == nil, customUnderlay == nil, selectedShareVideoURL == nil else { return }
        guard let session = resolvedSession else { return }
        if let preferred = SessionArtUnderlayPicker.preferredSource(for: session) {
            selectedUnderlay = preferred
        }
    }

    private func loadUnderlayImageForStudio(session: Session) -> UIImage? {
        if let existing = loadUnderlayImage() { return existing }
        let source = selectedUnderlay ?? SessionArtUnderlayPicker.preferredSource(for: session)
        return SessionArtUnderlayPicker.loadImage(session: session, source: source, uploaded: customUnderlay)
    }
    ...
    private var footnoteEditorField: some View {
        VStack(alignment: .leading, spacing: 10) {
            VStack(alignment: .leading, spacing: 6) {
                Text("Custom headline (movable in Live Studio)")
                    .font(.caption.bold())
                    .foregroundColor(.gray)
                TextField("Optional headline override", text: $previewCustomHeadline)
