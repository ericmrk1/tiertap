...
struct CloseoutView: View {
    ...
    @ViewBuilder
    private var avgBetCloseoutSection: some View {
        if TierTapProductEnhancements.isEnabled {
            VStack(alignment: .leading, spacing: 10) {
                L10nText("Average bet (optional)")
                    .font(.caption.bold())
                    .foregroundColor(.gray)
                Text("Actual vs host-rated — unlocks Tap Level, Rated Capture, and tiers/$100 analytics.")
                    .font(.caption2)
                    .foregroundColor(.white.opacity(0.65))
                InputRow(
                    label: "Avg bet actual",
                    placeholder: "What you bet",
                    value: $avgBetActual,
                    dialPadNavigationTitle: "Avg bet actual"
                )
                .environmentObject(settingsStore)
                InputRow(
                    label: "Avg bet rated",
                    placeholder: "Host / rated average",
                    value: $avgBetRated,
                    dialPadNavigationTitle: "Avg bet rated"
                )
                .environmentObject(settingsStore)
            }
        }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if TierTapProductEnhancements.isEnabled {
                ...
                if avgBetActual.isEmpty, let actual = defaults.actual {
                    avgBetActual = "\(actual)"
                }
                if avgBetRated.isEmpty, let rated = defaults.rated {
                    avgBetRated = "\(rated)"
                }
            }
    ...
    private func closeSessionPersistingLastGameDefaults(cashOut: Int, endingTier: Int, privateNotes: String?) -> TimeInterval? {
        ...
        let parsedActual = TierTapProductEnhancements.isEnabled ? Int(avgBetActual) : nil
        let parsedRated = TierTapProductEnhancements.isEnabled ? Int(avgBetRated) : nil
        ...
        store.closeSession(
            ...
            avgBetActual: parsedActual,
            avgBetRated: parsedRated
        )
