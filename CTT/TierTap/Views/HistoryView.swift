...
struct HistoryView: View {
    /// When `false`, History embeds inside Journal (no sheet Close / nested NavigationStack title chrome).
    ...
    @State private var selectedHistoryVerifications: Set<SessionTierPointsVerification> = []
    ...
    @State private var isHistoryVerificationSectionExpanded: Bool = false
    ...
    /// Sessions for the list and grid: date, location, game, verification, and search filters.
    private var filteredSessions: [Session] {
        ...
        if !selectedHistoryVerifications.isEmpty {
            sessions = sessions.filter { selectedHistoryVerifications.contains($0.effectiveTierPointsVerification) }
        }
    ...
    private var historyFiltersActive: Bool {
        useDateRangeFilter || !selectedHistoryGames.isEmpty || !selectedHistoryLocations.isEmpty ||
            !selectedHistoryVerifications.isEmpty ||
    ...
    private func clearHistoryFilters() {
        ...
        selectedHistoryVerifications.removeAll()
        searchText = ""
    }

    private func historyVerificationTitle(_ value: SessionTierPointsVerification) -> String {
        switch value {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isFilterPanelExpanded {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    historyVerificationBubblesSection
    ...
    private var historyVerificationBubblesSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            Button {
                withAnimation { isHistoryVerificationSectionExpanded.toggle() }
            } label: {
                HStack {
                    LocalizedLabel(title: "Verification", systemImage: "checkmark.seal")
                        .font(.caption)
                        .foregroundColor(.white.opacity(0.9))
                    Spacer()
                    if !selectedHistoryVerifications.isEmpty {
                        Text("\(selectedHistoryVerifications.count) selected")
                            .font(.caption2)
                            .foregroundColor(.green)
                    }
                    Image(systemName: isHistoryVerificationSectionExpanded ? "rectangle.compress.vertical" : "rectangle.expand.vertical")
                        .font(.caption2)
                        .foregroundColor(.white.opacity(0.7))
                }
            }
            ...
            if isHistoryVerificationSectionExpanded {
                HStack(spacing: 8) {
                    ForEach(SessionTierPointsVerification.allCases, id: \.self) { value in
                        let isSelected = selectedHistoryVerifications.contains(value)
                        Button {
                            if isSelected {
                                selectedHistoryVerifications.remove(value)
                            } else {
                                selectedHistoryVerifications.insert(value)
                            }
                        } label: {
                            Text(historyVerificationTitle(value))
                                .font(.caption)
                                .padding(.horizontal, 10)
                                .padding(.vertical, 6)
                                .background(isSelected ? Color.green : Color.white.opacity(0.18))
                    ...
                    SessionRow(session: session)
                        ...
                        .swipeActions(edge: .leading, allowsFullSwipe: false) {
                            ...
                            if session.effectiveTierPointsVerification == .unverified {
                                Button {
                                    var s = session
                                    s.tierPointsVerification = .verified
                                    store.updateSession(s)
                                } label: {
                                    LocalizedLabel(title: "Verify", systemImage: "checkmark.seal")
                                }
                                .tint(.blue)
                            }
                        }
    ...
    private var historyRoot: some View {
        ...
        .localizedNavigationTitle(presentsAsSheet ? "History" : "Journal")
