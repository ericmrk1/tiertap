...
private let keyCustomLadderTiers = "ctt_custom_ladder_tiers_v1"
...
private let keyShowTabBarLabels = "ctt_show_tab_bar_labels"
...
private let keyMonthlyWrappedNotificationsEnabled = "ctt_monthly_wrapped_notifications_enabled"
...
final class SettingsStore: ObservableObject {
    ...
    /// User-authored loyalty ladder status steps (label + points), shown in the Ladder goal picker.
    @Published var customLadderTiers: [CustomLadderTierEntry] {
        didSet {
            if customLadderTiers.isEmpty {
                UserDefaults.standard.removeObject(forKey: keyCustomLadderTiers)
            } else if let data = try? JSONEncoder().encode(customLadderTiers) {
                UserDefaults.standard.set(data, forKey: keyCustomLadderTiers)
            }
        }
    }
    ...
    /// When true, show one-word labels below icons in the main tab bar pill.
    @Published var showTabBarLabels: Bool {
        didSet { UserDefaults.standard.set(showTabBarLabels, forKey: keyShowTabBarLabels) }
    }
    ...
    /// When true (default), schedule a local notification when last month's Monthly Wrapped is ready.
    @Published var monthlyWrappedNotificationsEnabled: Bool {
        didSet { UserDefaults.standard.set(monthlyWrappedNotificationsEnabled, forKey: keyMonthlyWrappedNotificationsEnabled) }
    }
    ...
    init() {
        ...
        if let data = UserDefaults.standard.data(forKey: keyCustomLadderTiers),
           let decoded = try? JSONDecoder().decode([CustomLadderTierEntry].self, from: data) {
            self.customLadderTiers = decoded
        } else {
            self.customLadderTiers = []
        }
        ...
        if UserDefaults.standard.object(forKey: keyShowTabBarLabels) != nil {
            self.showTabBarLabels = UserDefaults.standard.bool(forKey: keyShowTabBarLabels)
        } else {
            self.showTabBarLabels = true
        }
        ...
        if UserDefaults.standard.object(forKey: keyMonthlyWrappedNotificationsEnabled) != nil {
            self.monthlyWrappedNotificationsEnabled = UserDefaults.standard.bool(forKey: keyMonthlyWrappedNotificationsEnabled)
        } else {
            self.monthlyWrappedNotificationsEnabled = true
        }
    ...
    /// Adds a custom ladder status for a program. Returns `false` if label/points are invalid or duplicate.
    @discardableResult
    func addCustomLadderTier(programName: String, label: String, points: Int) -> Bool {
        let program = programName.trimmingCharacters(in: .whitespacesAndNewlines)
        let tierLabel = label.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !program.isEmpty, !tierLabel.isEmpty, points > 0 else { return false }

        // Prefer canonical built-in program spelling when available.
        let canonicalProgram = TierTapProductEnhancements.ladder(matchingProgram: program)?.programName ?? program
        let programKey = canonicalProgram.lowercased()
        let labelKey = tierLabel.lowercased()

        if let builtIn = TierTapProductEnhancements.ladder(matchingProgram: canonicalProgram),
           builtIn.tiers.contains(where: {
               $0.points == points || $0.label.lowercased() == labelKey
           }) {
            return false
        }

        let duplicate = customLadderTiers.contains {
            $0.programName.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == programKey
                && (
                    $0.points == points
                        || $0.label.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == labelKey
                )
        }
        guard !duplicate else { return false }

        customLadderTiers.append(
            CustomLadderTierEntry(programName: canonicalProgram, label: tierLabel, points: points)
        )
        rememberRewardProgramName(canonicalProgram)
        return true
    }

    func removeCustomLadderTier(id: UUID) {
        customLadderTiers.removeAll { $0.id == id }
    }
