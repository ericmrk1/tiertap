...
final class AuthStore: ObservableObject {
    ...
    var userProfilePhotoURL: URL? {
        ...
        return try? client.storage.from("avatars").getPublicURL(path: Self.communityAvatarStoragePath(for: userId))
    }

    /// OAuth / OpenID profile picture from the sign-in provider (commonly Google `picture`).
    /// Apple Sign In does not provide a photo. Not written into JWTs by TierTap.
    var oauthAccountProfilePhotoURL: URL? {
        for key in ["picture", "avatar_url"] {
            if let raw = stringFromUserMetadata(key)?.trimmingCharacters(in: .whitespacesAndNewlines),
               !raw.isEmpty,
    ...
    var hasLocalProfilePhoto: Bool { localProfilePhotoImage != nil }
    var hasOAuthAccountProfilePhoto: Bool { oauthAccountProfilePhotoURL != nil }
    var hasAnyShareableProfilePhoto: Bool { hasLocalProfilePhoto || hasOAuthAccountProfilePhoto }
    ...
    /// Stable Storage object path for Community feed avatars (`{userId}/avatar.jpg`).
    /// Uses lowercase UUID so the path matches `auth.uid()::text` in Storage RLS policies.
    nonisolated static func communityAvatarStoragePath(for userId: UUID) -> String {
    ...
    /// - Returns: Storage object path suitable for Community `session_details.avatar_path`.
    @discardableResult
    func uploadProfilePhoto(_ imageData: Data) async throws -> String {
        ...
        let path = Self.communityAvatarStoragePath(for: userId)
    ...
    /// Uploads the chosen Community share photo into `avatars` and returns the storage path stamped onto posts.
    func uploadAvatarForCommunityShare(source: CommunityProfilePhotoShareSource) async throws -> String {
        let imageData: Data
        switch source {
        case .tierTap:
            guard let image = localProfilePhotoImage,
                  let data = image.jpegData(compressionQuality: 0.8) else {
                throw AuthError.profilePhotoUnavailable
            }
            imageData = data
        case .signInAccount:
            guard let url = oauthAccountProfilePhotoURL else {
                throw AuthError.profilePhotoUnavailable
            }
            do {
                let (data, response) = try await URLSession.shared.data(from: url)
                if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
                    throw AuthError.profilePhotoDownloadFailed
                }
                guard !data.isEmpty else { throw AuthError.profilePhotoDownloadFailed }
        ...
        return try await uploadProfilePhoto(imageData)
    }
...
enum AuthError: LocalizedError {
    ...
    case profilePhotoUnavailable
    case profilePhotoDownloadFailed
    ...
    var errorDescription: String? {
        ...
        case .profilePhotoUnavailable:
            return "No profile photo is available to share. Set one in Account, or use a Google account photo."
        case .profilePhotoDownloadFailed:
            return "Could not download your sign-in account photo. Try your TierTap profile photo instead."
...
/// Which photo to copy into the Community `avatars` bucket when sharing on publish.
enum CommunityProfilePhotoShareSource: String, CaseIterable, Identifiable, Hashable {
    case tierTap
    case signInAccount
    ...
    var title: String {
        switch self {
        case .tierTap: return "TierTap photo"
        case .signInAccount: return "Sign-in account photo"
        }
    }
}
