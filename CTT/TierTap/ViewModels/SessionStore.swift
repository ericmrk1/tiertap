...
class SessionStore: ObservableObject {
    ...
    /// Brief centered toast after publishing sessions to Community (auto-cleared by the UI).
    @Published var communityPublishToastMessage: String?
    #if os(iOS)
    ...
    init() {
        ...
        SessionSyncManager.shared.tierGoalsSnapshotProvider = {
            SessionSyncManager.shared.latestTierGoals()
        }
        #endif
        setupSync()
        #if os(iOS)
    ...
    func closeSession(
        ...
        presentPostCloseoutSharePrompt: Bool = true,
        avgBetActual: Int? = nil,
        avgBetRated: Int? = nil
    ) {
        guard var s = liveSession else { return }
        s.cashOut = cashOut
        s.liveTrackedStackAmount = nil
        s.avgBetActual = avgBetActual
        s.avgBetRated = avgBetRated
        s.endingTierPoints = endingTier
    ...
    private func reloadTierTapComplicationTimelines() {
        ...
        WidgetCenter.shared.reloadTimelines(ofKind: "TierTapRewardsGoalsWidget")
        #endif
    }
    ...
    private func publishHomeWidgetSnapshot() {
        ...
        let snapshot = TierTapWidgetSnapshotBuilder.build(
            ...
            tierGoals: Self.loadWalletTierGoalsForWidget()
        )
        TierTapWidgetSnapshotStore.save(snapshot)
    }

    /// Reads wallet cards from disk so widgets can show every tier goal ring.
    private static func loadWalletTierGoalsForWidget() -> [TierTapWidgetTierGoal] {
        guard TierTapProductEnhancements.isEnabled else { return [] }
        guard let documents = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
            return []
        }
        let url = documents
            .appendingPathComponent("reward_wallet", isDirectory: true)
            .appendingPathComponent("cards.json")
        guard let data = try? Data(contentsOf: url),
              let cards = try? JSONDecoder().decode([RewardWalletCard].self, from: data) else {
            return []
        }
        return cards
            .compactMap { card -> TierTapWidgetTierGoal? in
                guard card.hasTierGoal, let goal = card.tierGoalPoints else { return nil }
